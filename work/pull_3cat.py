...
CATS={"carpet":"17028727","hat":"41777465","suitcase":"17027904"}
with httpx.Client(base_url="https://api-seller.ozon.ru",headers=h,timeout=40) as c:
    ...
    for j in range(0,len(pids),900):
        ...
        for it in r.get("result",[]):
            ...
            for k,want in CATS.items():
                if cid==want and len(buckets[k])<15:
    ...
    for k,items in buckets.items():
        ...
        Path(rf"C:\MvideoERP\work\{k}_input.json").write_text(json.dumps(items,ensure_ascii=False,indent=1),encoding="utf-8")
        print(k,len(items))
