...
class OzonSellerClient:
    ...
    def attach_video_covers(self, *, offer_ids: list[str], video_cover_url: str) -> dict[str, Any]:
        """Attach a short MP4 video cover (Ozon complex 100002 / attribute
        21845) to existing products via /v1/product/attributes/update.  This is
        an optional CTR/rating enhancement: a failure here never blocks the
        product card itself."""
        if not offer_ids:
            raise ValueError("at least one offer_id is required")
        normalized = [str(o).strip() for o in offer_ids if str(o).strip()]
        if not normalized:
            raise ValueError("at least one offer_id is required")
        video_cover_url = (video_cover_url or "").strip()
        if not video_cover_url.startswith(("http://", "https://")):
            raise ValueError("video_cover_url must be a valid http(s) link")
        items: list[dict[str, Any]] = []
        for oid in normalized:
            items.append({
                "offer_id": oid,
    ...
    def import_product_pictures(self, *, offer_id: str, images: list[str]) -> dict[str, Any]:
        """Import picture URLs into an existing Ozon product card (v2).

        Iteration 2026-09-16: Ozon 已废弃 /v1/product/pictures/import，
        现行 /v2/product/pictures/import 使用 items[{offer_id, images}] 结构。
        旧 v1 调用恒定返回 400 VALIDATION ERROR，导致前端一直报"图片链接无效"。
        """
        if not offer_id:
        ...
        if len(valid) > 15:
            valid = valid[:15]
        return self._post("/v2/product/pictures/import",
                          {"items": [{"offer_id": str(offer_id), "images": valid}]})
